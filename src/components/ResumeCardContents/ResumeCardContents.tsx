...
const formatContentLinkTitle = (content: CardContent) => (
  <a className="--gth-underline" href={content.link}>
    {content.title}
  </a>
);
const formatContentTitle = (content: CardContent) => (
  <span className="--gth-underline">{content.title}</span>
);
...
const formatAccomplishments = (accomplishments: CardAccomplishment[] | undefined) => {
  ...
  return <ul>{accomplishments.map((a, i) => <li key={i}>{a.text}</li>)}</ul>;
};

const ResumeCardContents: React.StatelessComponent<Props> = (props: Props) => {
  const content = props.cardContent;
  const formattedTitle = content.link
    ? formatContentLinkTitle(content)
    : formatContentTitle(content);
  return (
    <div className="gth-card-contents">
      <AdminLinkContainer to={`/resume/${content.key}/edit`}>Edit</AdminLinkContainer>
      <div className="gth-card-header">
        <h3>{formattedTitle}</h3>
        <p>
          <code>{content.date}</code>
        </p>
      </div>
